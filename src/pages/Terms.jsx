...
export default function Terms() {
  const { t } = useLanguage()
  const sections = ['use', 'form', 'ownership']
  ...
  return (
    <div className="min-h-screen bg-background text-ink font-body px-6 sm:px-10 lg:px-16 py-16 max-w-3xl mx-auto">
      ...
      <div className="space-y-8 text-[15px] leading-relaxed text-ink/80">
        {sections.map((key) => (
          <section key={key}>
            <h2 className="font-display font-bold text-xl text-ink mb-2">{t(`legal.terms.sections.${key}.title`)}</h2>
            <p>{t(`legal.terms.sections.${key}.text`)}</p>
          </section>
        ))}
        <section>
